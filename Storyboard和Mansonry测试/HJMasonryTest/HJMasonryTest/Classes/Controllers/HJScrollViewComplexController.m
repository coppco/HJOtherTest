//
//  HJScrollViewComplexController.m
//  HJMasonryTest
//
...
#import "HJScrollViewComplexController.h"

@interface HJScrollViewComplexController ()

@end

@implementation HJScrollViewComplexController
...
//        make.top.mas_equalTo(scrollView).offset(20);
        ...
        warning.text =@"mas_equalTo和equalTo区别：前者比后者多了类型转换操作，支持CGSize CGPoint NSNumber UIEdgeinsets。mas_equalTo是equalTo的封装，equalTo适用于基本数据类型，而mas_equaalTo适用于类似UIEdgeInsetsMake 等复杂类型，基本上它可以替换equalTo, 当使用scrollview的时候,第一个子控件不能使用mas_equalTo(self.view)或者equalTo(self.view),可以是scrollview";
    }];
