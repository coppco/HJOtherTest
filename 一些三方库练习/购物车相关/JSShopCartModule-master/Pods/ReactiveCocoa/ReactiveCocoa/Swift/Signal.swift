...
extension SignalType {
	...
	/// itself.
	public func observeNext(_ next: (Value) -> ()) -> Disposable? {
	...
	/// callback. Disposing of the Disposable will have no effect on the Signal
	/// itself.
	public func observeFailed(_ error: (Error) -> ()) -> Disposable? {
			...
			disposable += self.observe { event in
				if case let .next(value) = event {
					...
					state.modify { state in
						...
						let proposedScheduleDate = mutableState.previousDate?.addingTimeInterval(interval) ?? scheduler.currentDate
