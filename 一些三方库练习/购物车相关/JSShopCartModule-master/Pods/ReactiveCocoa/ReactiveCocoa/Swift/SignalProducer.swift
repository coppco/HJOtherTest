...
public struct SignalProducer<Value, Error: ErrorType> {
	...
	/// `Interrupted` event.
	public func startWithSignal(@noescape _ setUp: (Signal<Value, Error>, Disposable) -> ()) {
...
public protocol SignalProducerType {
	...
	/// Creates a Signal from the producer, passes it into the given closure,
	/// then starts sending events on the Signal when the closure has returned.
	func startWithSignal(@noescape _ setUp: (Signal<Value, Error>, Disposable) -> ())
}
...
extension SignalProducerType {
	...
	public func startWithNext(_ next: (Value) -> ()) -> Disposable {
	...
	/// Returns a Disposable which can be used to interrupt the work associated
	/// with the Signal.
	public func startWithFailed(_ failed: (Error) -> ()) -> Disposable {
	...
	@warn_unused_result(message="Did you forget to call `start` on the producer?")
	fileprivate func liftRight<U, F, V, G>(_ transform: (Signal<Value, Error>) -> (Signal<U, F>) -> Signal<V, G>) -> (SignalProducer<U, F>) -> SignalProducer<V, G> {
	...
	/// to generate correct results.
	@warn_unused_result(message="Did you forget to call `start` on the producer?")
	fileprivate func liftLeft<U, F, V, G>(_ transform: (Signal<Value, Error>) -> (Signal<U, F>) -> Signal<V, G>) -> (SignalProducer<U, F>) -> SignalProducer<V, G> {
		return { otherProducer in
			return SignalProducer { observer, outerDisposable in
				otherProducer.startWithSignal { otherSignal, otherDisposable in
					outerDisposable.addDisposable(otherDisposable)
